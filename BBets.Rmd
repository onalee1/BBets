...
1. What are the fair odds for a bet on matching birthdays in a 35 person class.
  ...
  Our plan is we will use a simulation where we hire 40 different brownlows and send them all out to try this and manipulate the value of odds until the averge of the different value comes out to zero.
```{r}
profit=as.numeric()
for (i in 1:400) {
...
hist(profit)
```
   According to the histrogram, the fair odds are at around 4.4 to 4.5 to match a class of 35 students. This is where browlow is not making any profit.
...
hist(profit)
```
   For a 2:1 fair odds bet in the classroom, the size should be around approximately between 28 or 29 students so it can be $2 bet outta $1 each students
...
print(sprofit [975])
```
   According to the R.Console there a 95% chance that the low end will be between 200 to 230 as low, than a 95% chance of high end where it goes from  a 330 to 400 chance in the bets of how many students in class will have the same birthday.
